<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['update'])) {
        ...
        // check mail
        $checkmail = $connect->prepare("SELECT id FROM users WHERE email = :email AND id != :id");
        $checkmail->execute([
            ':email' => $email,
            ':id' => $id
        ]);
            ...
            if ($nameflag && $emailflag && $passwordflag && $genderflag && $roleflag && $interestflag && $addressflag) {
                ...
                //$_SESSION['msg'] = "<div class='alert alert-info' role='alert' id='alert'>record update successfully</div>";
                  ...
                  $_SESSION['toast'] = "
                        ...
                        <div class='toast show' role='alert' id='toastmsg' aria-live='assertive' aria-atomic='true'>
